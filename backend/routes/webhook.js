...
const { handleRestaurantBotMessage } = require('../services/restaurantBot');
    ...
    for (const entry of body.entry || []) {
      for (const change of entry.changes || []) {
        ...
        for (const message of messages) {
          ...
          await processMessage(phoneNumberId, message, value, contacts);
        }
      }
    }
  } catch (err) {
...
const processMessage = async (phoneNumberId, message, value, contacts = []) => {
  console.log("─────────────────────────────────");
  console.log("📨 phoneNumberId    :", phoneNumberId);
  ...
  if (phoneNumberId === mainPhoneNumberId) {
    ...
    if (waConfig) {
      ...
      console.log(
        "ℹ️ This number has no active restaurant bot. Add it via Admin → Manual Activate",
      );
    }
  }
};
